...
def run_simulation(robot_id, num_robots, use_gui=True, coord_log=None, log_lock=None):
    """
    ...
    try:
        while True:
            ...
            for qdrp in robots:
                ...
                local_log.append(targetPositionRF.tolist())

                # Advance simulation
                qdrp.oneStep()
            
            if coord_log is not None and log_lock is not None:
                with log_lock:
                    coord_log[robot_id]["RF"].extend(local_log)
                local_log.clear()
...
def main():
    ...
    coord_log = manager.dict({
        i: { "RF": manager.list() } for i in range(num_simulations)
    })
